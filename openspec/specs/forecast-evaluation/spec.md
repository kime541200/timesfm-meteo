...
把 TimesFM forecast 結果持久化到 Postgres，並在指定範圍內把 forecast 與後續觀測值對齊，依 horizon_step 分組計算 max / min 各自的 MAE、interval coverage 與 interval width。為 live workflow（今天預測、幾天後比對）與 backtest workflow（用過去日期重跑、立刻比對）提供統一的評估閉環。
